...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1t58veLEnL?p=114)


# 第一章 绪论
## 1.1 选题背景
目前整个社会发展的速度，严重依赖于互联网，如果没有了互联网的存在，市场可能会一蹶不振，严重影响经济的发展水平，影响人们的生活质量。计算机的发展，不管是从硬件还是软件，都有很多技术储备，每年都有很多的技术和软件产生，纵观各个领域，无一不用互联网软件，办公用的还是电脑居多，但是人手一台智能设备已经变成了现在人们的生活日常，哪怕的普通的老百姓都成了上知天文下知地理的所在，这些都依赖于互联网技术。互联网技术在信息的传播方面是巨大的，而信息的处理就变成了各种产业管理者和维护者的重任。人们已经习惯了互联网的存在，所以经常操作一些互联网产品变成了日常，所以开发一个物资物流系统，让人们使用是没有一点问题的，并且在这个过程中不仅能够规范物资物流信息管理的使用流程还有信息处理流程，也能让整个信息的传播处理，都存在一种可控制的范围，最重要的是，计算机在物资物流信息管理方面可以给管理者提供更好的帮助。
## 1.2 选题意义
当开发软件变成了一个潮流之后，会发现不管任何行业都能开发适合自己的软件，不管是内部员工管理，还是财务管理，甚至业务管理都可以数据化，并且可以对这些数据集中处理，进而根据数据处理结果就自然而然的提高了管理水平，最重要的是，开发一个软件投入使用，开发过程其实就是梳理行业痛点的过程，就是让软件来弥补行业的管理漏洞，不断的优化事物的处理流程。物资物流系统就是一款专门开发的软件，通过Web技术，让使用者可以在任何一台智能设备上面通过浏览器进行操作使用，对数据的处理不在局限于地域距离，只要软件开发到位，使用起来方便，达到预期目的，会发现有软件和没有软件的区别是很大的，有了这款软件之后，会发现数据的存储安全方面，比起之前的满屋寻找记录的优势是多么的巨大。
## 1.3 研究内容
...
管理员用例图的绘制结果见图3-5。管理员登录进入本系统操作的功能包括管理车辆类型，管理司机，管理承运任务，管理公告，管理车辆等。

![](/md/blog.005.png)

图3-5 管理员用例图

司机用例图的绘制结果见图3-6。司机登录进入本系统操作的功能包括查看车辆，查看承运任务信息，查看公告，更改密码和个人信息。
...
管理员功能结构图的绘制结果见图4-1。管理员登录进入本系统操作的功能包括管理车辆类型，管理司机，管理承运任务，管理公告，管理车辆等。

![](/md/blog.007.png)

图4-1 管理员功能结构图

司机功能结构图的绘制结果见图4-2。司机登录进入本系统操作的功能包括查看车辆，查看承运任务信息，查看公告，更改密码和个人信息。
...
（1）使用Visio这样的常用的实体属性图绘制工具来绘制司机实体属性图，绘制结果见图4-3。
...
（3）使用Visio这样的常用的实体属性图绘制工具来绘制承运任务实体属性图，绘制结果见图4-5。

![](/md/blog.011.png)

图4-5 承运任务实体属性图

（4）使用Visio这样的常用的实体属性图绘制工具来绘制车辆实体属性图，绘制结果见图4-6。
...
通过上节内容可以知晓数据库中的各个实体，并通过一定方式把这些实体表示的内容进行数据表的转换，通常来说，每个实体都会对应一张具体的数据表，在本系统指定的数据库中创建命名好的数据库，才可以对数据表进行创建与设计。物资物流系统数据表设计结果展示如下：
...
## 5.1管理员功能实现
### 5.1.1 承运任务管理
管理员进入如图5-1所示的承运任务管理界面之后，管理员点击信息显示栏中最右侧的详情，删除按钮可依次完成承运任务信息的详情查看，删除操作，对于处于未开始状态的承运任务信息可以进行修改，可以设置为开始运输。

![](/md/blog.014.png)

图5-1 承运任务管理界面
### 5.1.2 司机管理
管理员进入如图5-2所示的司机管理界面之后，管理员点击信息显示栏中最右侧的修改，删除，重置密码按钮可依次完成司机信息的修改，删除，账号密码重置等操作，管理员在当前界面也能添加司机，查询司机。

![](/md/blog.015.png)

图5-2 司机管理界面
### 5.1.3 车辆管理
管理员进入如图5-3所示的车辆管理界面之后，管理员点击信息显示栏右侧的修改，删除按钮可依次完成车辆信息的修改，删除等操作。

![](/md/blog.016.png)

图5-3 车辆管理界面
### 5.1.4 公告信息管理
管理员进入如图5-4所示的公告信息管理界面之后，管理员点击信息显示栏中最右侧的修改，删除按钮可依次完成公告信息的修改，删除等操作。
...
## 5.2司机功能实现
### 5.2.1 承运任务查看
司机进入如图5-5所示的承运任务查看界面之后，司机可以对承运任务的运单状态进行查看，有已完成状态，也有未开始状态，司机点击信息显示栏右侧的详情按钮可以对承运任务的详情信息进行查看。

![](/md/blog.018.png)

图5-5 承运任务查看界面
### 5.2.2 公告信息查看
司机进入如图5-6所示的公告信息查看界面之后，司机可以查询公告，可以通过详情按钮的点击进入公告详情界面对公告具体内容进行查看。

![](/md/blog.019.png)

图5-6 公告信息查看界面
### 5.2.3 车辆信息查看
司机进入如图5-7所示的车辆信息查看界面之后，司机查看车牌号，车辆所属类型，车辆的排量信息以及司机联系方式信息等信息。
